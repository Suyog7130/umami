...
class Test:
    ...
    def test(self):
        ...
        for (x, labels, keys, strains, phases, attr) in tqdm(iter(self.test_loader)):
...
def plot_mismatch(original, reconst, labels, keys, reshape2orig=False, savedir='../results/',
    ...
    Returns
    --------
    mismatch_amp : np.ndarray
        Mismatch values for the amplitude.
    mismatch_freq : np.ndarray
        Mismatch values for the frequency.
    chirpmasses : np.ndarray
        Chirp masses calculated from the labels.
    totalmasses : np.ndarray
        Total masses calculated from the labels.
    massratios : np.ndarray
        Mass ratios calculated from the labels.
    ...
    mismatch_amp = np.zeros((original.shape[0], 1))
    mismatch_freq = np.zeros((original.shape[0], 1))
    ...
    for i in range(original.shape[0]):
        ...
        if reshape2orig:
            # Reshape to original data shape
            orig_data = original[i].reshape([2,PRESET_ARRAY_SIZE])
            reconst = reconst.reshape([2,PRESET_ARRAY_SIZE])
        else:
            # Use the original shape of the data
            logging.debug(f"original shape: {original.shape}, reconst shape: {reconst.shape}")
            orig_data = original[i].reshape([2,original.shape[2]])
            recon_data = reconst[i].reshape([2,reconst.shape[2]])
...
def plot_polarization_mismatch(original, reconst, labels, keys, phases, strains, attr,
                               reshape2orig=False,
    ...
    Returns
    -------
    mismatch_hplus : np.ndarray
        Mismatch values for the h_plus polarization.
    mismatch_hcross : np.ndarray
        Mismatch values for the h_cross polarization.
    chirpmasses : np.ndarray
        Chirp masses calculated from the labels.
    totalmasses : np.ndarray
        Total masses calculated from the labels.
    massratios : np.ndarray
        Mass ratios calculated from the labels.
    chieffs : np.ndarray
        Effective spin parameters calculated from the labels (if available).
    num_saved_overplots : int
        Number of random samples to save overplot visualizations for.
    ...
    logging.debug(f"original shape: {original.shape}, reconst shape: {reconst.shape}, phases shape: {phases.shape}, strains shape: {strains.shape}")

    chirpmasses = np.zeros((labels.shape[0], 1))  # Store chirp masses for each sample
    totalmasses = np.zeros((labels.shape[0], 1))  # Store total masses for each sample
    massratios = np.zeros((labels.shape[0], 1))  # Store mass ratios for each sample
    mismatch_hplus = np.zeros((original.shape[0], 1))
    mismatch_hcross = np.zeros((original.shape[0], 1))
